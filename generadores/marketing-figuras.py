...
def fm1():
    ...
    BANDAS = [
        (62, "Fuera · todavía no nos busca", ["E1 Ajeno", "E2 Latente", "E3 Despierto"], "#7B8A91", False),
    ...
    for y, rotulo, nodos, color, punteado in BANDAS:
        ...
        for n in nodos:
            ...
            p.append('<rect x="%d" y="%d" width="%d" height="%d" rx="3" fill="%s" stroke="%s" '
                     'stroke-width="1.5"%s/>'
                     % (x, y, CAJA_W, CAJA_H, "var(--surface)" if punteado or color == "#7B8A91" else "var(--paper)",
    ...
    p.append('<defs>'
             ...
             '<path d="M0,0 L10,5 L0,10 z" fill="%s"/></marker>'
             '</defs>' % (TEAL, "#7B8A91", OCRE))

    def horizontal(a, b, color="#7B8A91", grosor=1.4, marca="pm-g", guion=""):
        A, B = pos[a], pos[b]
        return ('<path d="M%s %s H%s" fill="none" stroke="%s" stroke-width="%s"%s '
                'marker-end="url(#%s)"/>'
                % (A["x2"] + 3, A["y"] + CAJA_H / 2, B["x"] - 9, color, grosor, guion, marca))

    def codo(a, b, color="#7B8A91", grosor=1.4, marca="pm-g", guion="", desvio=0, entrada=0):
